...
results_dir = "blue_banner_epa_results"
...
# for each blue banner count, print out the percentage of teams who had a match progression below one in that category
for blue_banners in np.sort(data["blue_banners"].unique()):
    ...
    below_one_percentage = (
        (blue_banners_data["elim_progression"] < 1).sum() / len(blue_banners_data) * 100
    )
    print(
        f"For teams with {blue_banners} blue banners, {below_one_percentage:.2f}% had an elim_progression below 1."
    )


# Print out teams and years with highest norm_epa who had 0 blue banners
top_teams_no_blue_banners = data[
    (data["blue_banners"] == 0) & (data["year"] > 2018)
].nlargest(30, "norm_epa")[["team", "year", "norm_epa"]]

print("\nHigh EPA Teams with 0 Blue Banners:")
print(top_teams_no_blue_banners)


# print out teams and years with lowest norm_epa who had 1 blue banner
top_teams_no_blue_banners = data[
    (data["blue_banners"] == 1) & (data["year"] > 2018)
].nsmallest(30, "norm_epa")[["team", "year", "norm_epa"]]

print("\nLow EPA Teams with 1 Blue Banners:")
print(top_teams_no_blue_banners)


top_teams_no_blue_banners = data[
    (data["blue_banners"] == 5) & (data["year"] > 2018)
].nsmallest(30, "norm_epa")[["team", "year", "norm_epa"]]

print("\nLow EPA Teams with 5 Blue Banners:")
print(top_teams_no_blue_banners)
